package com.ironsource;

import com.ironsource.AbstractC2406k3;
import com.ironsource.mediationsdk.logger.IronSourceError;

/* JADX INFO: compiled from: r8-map-id-84874db269549a40c0b5c7061a31fb3953e4b1b5018e77414ceb6004f20237e9 */
/* JADX INFO: loaded from: classes5.dex */
public final class Ma implements Qa {

    /* JADX INFO: renamed from: a, reason: collision with root package name */
    private final Ga f7129a;

    /* JADX INFO: renamed from: b, reason: collision with root package name */
    private final M2 f7130b;

    public Ma(Ga strategy, M2 currentAdUnit) {
        kotlin.jvm.internal.k.e(strategy, "strategy");
        kotlin.jvm.internal.k.e(currentAdUnit, "currentAdUnit");
        this.f7129a = strategy;
        this.f7130b = currentAdUnit;
    }

    @Override // com.ironsource.L0
    public final /* synthetic */ void a() {
        mh.a(this);
    }

    @Override // com.ironsource.L0
    public final /* synthetic */ void b(C2544s0 c2544s0) {
        mh.b(this, c2544s0);
    }

    @Override // com.ironsource.Qa
    public void c() {
        this.f7130b.a(true);
        Ga ga = this.f7129a;
        ga.a((Qa) new Ja(ga));
    }

    @Override // com.ironsource.Qa
    public void e() {
        this.f7129a.a(this.f7130b, new AbstractC2406k3.a(AbstractC2406k3.b.c.f8820a));
    }

    @Override // com.ironsource.L0
    public void a(C2544s0 adUnitCallback) {
        kotlin.jvm.internal.k.e(adUnitCallback, "adUnitCallback");
        this.f7129a.a("Received load success while paused");
    }

    @Override // com.ironsource.Qa
    public void b() {
        this.f7129a.a("Loading a loaded ad");
    }

    @Override // com.ironsource.L0
    public void a(IronSourceError ironSourceError) {
        this.f7129a.a("Received load failed while paused");
    }

    @Override // com.ironsource.Qa
    public void d() {
    }
}
