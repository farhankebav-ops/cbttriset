...
public class ResponseValidator {
    ...
    private static void verifySignature(String signedData, String signature, String signatureAlgorithm, PublicKey publicKey) throws LicenseCheckException {
        try {
            ...
            throw new LicenseCheckException(String.format("Could not find %s algorithm on the device", signatureAlgorithm), e8);
        } catch (SignatureException e9) {
            throw new LicenseCheckException("Could not parse returned signature.", e9);
        }
    }
    ...
    private static PublicKey getPublicKey() throws LicenseCheckException {
        try {
            ...
            throw new LicenseCheckException("Could not create key specification from the public key", e8);
        }
    }
