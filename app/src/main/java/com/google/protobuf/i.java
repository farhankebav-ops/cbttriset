...
public final class i {
    ...
    private static long getChannelFieldOffset(Class<?> cls) {
        ...
        try {
            if (r2.hasUnsafeArrayOperations()) {
                return r2.objectFieldOffset(cls.getDeclaredField("channel"));
            }
            return -1L;
    ...
    private static boolean needToReallocate(int i2, int i8) {
        return i8 < i2 && ((float) i8) < ((float) i2) * 0.5f;
    }
    ...
    private static boolean writeToChannel(ByteBuffer byteBuffer, OutputStream outputStream) throws IOException {
        ...
        try {
            writableByteChannel = (WritableByteChannel) r2.getObject(outputStream, j);
