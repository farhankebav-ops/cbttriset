...
public class CrashlyticsReportDataCapture {
    ...
    static {
        ...
        a0.j(5, map, "armeabi", 6, "armeabi-v7a");
        a0.j(9, map, "arm64-v8a", 0, "x86");
        map.put("x86_64", 1);
    ...
    private CrashlyticsReport.Session.Event.Application populateEventApplicationData(int i2, TrimmedThrowableData trimmedThrowableData, Thread thread, int i8, int i9, boolean z2) {
        Boolean boolValueOf;
    ...
    private CrashlyticsReport.Session.Event.Application.Execution.Exception populateExceptionData(TrimmedThrowableData trimmedThrowableData, int i2, int i8) {
        return populateExceptionData(trimmedThrowableData, i2, i8, 0);
    }

    private CrashlyticsReport.Session.Event.Application.Execution populateExecutionData(TrimmedThrowableData trimmedThrowableData, Thread thread, int i2, int i8, boolean z2) {
        return CrashlyticsReport.Session.Event.Application.Execution.builder().setThreads(populateThreadsList(trimmedThrowableData, thread, i2, z2)).setException(populateExceptionData(trimmedThrowableData, i2, i8)).setSignal(populateSignalData()).setBinaries(populateBinaryImagesList()).build();
    }
    ...
    public CrashlyticsReport.Session.Event captureEventData(Throwable th, Thread thread, String str, long j, int i2, int i8, boolean z2) {
        int i9 = this.context.getResources().getConfiguration().orientation;
        return CrashlyticsReport.Session.Event.builder().setType(str).setTimestamp(j).setApp(populateEventApplicationData(i9, TrimmedThrowableData.makeTrimmedThrowableData(th, this.stackTraceTrimmingStrategy), thread, i2, i8, z2)).setDevice(populateEventDeviceData(i9)).build();
    }
    ...
    private CrashlyticsReport.Session.Event.Application.Execution.Exception populateExceptionData(TrimmedThrowableData trimmedThrowableData, int i2, int i8, int i9) {
        ...
        CrashlyticsReport.Session.Event.Application.Execution.Exception.Builder overflowCount = CrashlyticsReport.Session.Event.Application.Execution.Exception.builder().setType(str).setReason(str2).setFrames(populateFramesList(stackTraceElementArr, i2)).setOverflowCount(i10);
        if (trimmedThrowableData2 != null && i10 == 0) {
            overflowCount.setCausedBy(populateExceptionData(trimmedThrowableData2, i2, i8, i9 + 1));
        }
