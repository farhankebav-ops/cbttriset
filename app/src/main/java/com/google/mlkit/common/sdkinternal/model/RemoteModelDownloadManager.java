...
public class RemoteModelDownloadManager {
    ...
    public RemoteModelDownloadManager(@NonNull MlKitContext mlKitContext, @NonNull RemoteModel remoteModel, @NonNull ModelFileHelper modelFileHelper, @NonNull RemoteModelFileManager remoteModelFileManager, @Nullable ModelInfoRetrieverInterop modelInfoRetrieverInterop, @NonNull zzsh zzshVar) {
        ...
        DownloadManager downloadManager = (DownloadManager) mlKitContext.getApplicationContext().getSystemService(NativeAdPresenter.DOWNLOAD);
    ...
    public final MlKitException zzl(@Nullable Long l) {
        ...
        int i2 = 13;
        String strH = "Model downloading failed";
        if (cursorQuery != null && cursorQuery.moveToFirst()) {
            int i8 = cursorQuery.getInt(cursorQuery.getColumnIndex("reason"));
            if (i8 == 1006) {
                strH = "Model downloading failed due to insufficient space on the device.";
                i2 = 101;
            } else {
                strH = a.h(i8, "Model downloading failed due to error code: ", " from Android DownloadManager");
            }
        }
        return new MlKitException(strH, i2);
    }
    ...
    /* JADX WARN: Removed duplicated region for block: B:22:0x0049 A[Catch: all -> 0x003c, TRY_ENTER, TryCatch #2 {all -> 0x003c, blocks: (B:9:0x0027, B:11:0x002d, B:22:0x0049, B:24:0x0050, B:26:0x0057, B:28:0x005d, B:30:0x0065), top: B:50:0x0027, outer: #0 }] */
