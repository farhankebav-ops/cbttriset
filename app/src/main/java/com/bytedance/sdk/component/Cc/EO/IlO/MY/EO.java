package com.bytedance.sdk.component.Cc.EO.IlO.MY;

import com.bytedance.sdk.component.Cc.es;

/* JADX INFO: compiled from: r8-map-id-84874db269549a40c0b5c7061a31fb3953e4b1b5018e77414ceb6004f20237e9 */
/* JADX INFO: loaded from: classes3.dex */
public class EO implements es {
    private com.bytedance.sdk.component.Cc.EO.IlO.EO<String, byte[]> EO;
    private int IlO;
    private int MY;

    public EO(int i2, int i8) {
        this.MY = i2;
        this.IlO = i8;
        this.EO = new com.bytedance.sdk.component.Cc.EO.IlO.EO<String, byte[]>(i2) { // from class: com.bytedance.sdk.component.Cc.EO.IlO.MY.EO.1
            @Override // com.bytedance.sdk.component.Cc.EO.IlO.EO
            /* JADX INFO: renamed from: IlO, reason: merged with bridge method [inline-methods] */
            public int MY(String str, byte[] bArr) {
                if (bArr == null) {
                    return 0;
                }
                return bArr.length;
            }
        };
    }

    @Override // com.bytedance.sdk.component.Cc.IlO
    public boolean MY(String str) {
        return this.EO.IlO(str) != null;
    }

    @Override // com.bytedance.sdk.component.Cc.IlO
    public boolean IlO(String str, byte[] bArr) {
        if (str == null || bArr == null) {
            return false;
        }
        this.EO.IlO(str, bArr);
        return true;
    }

    @Override // com.bytedance.sdk.component.Cc.IlO
    public byte[] IlO(String str) {
        return this.EO.IlO(str);
    }
}
