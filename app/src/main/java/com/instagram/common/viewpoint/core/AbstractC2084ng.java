...
public abstract class AbstractC2084ng {
    ...
    public static <E> ArrayList<E> A05(Iterator<? extends E> elements) {
        ...
        AbstractC2063nL.A0B(list, elements);
        return list;
    }

    public static boolean A06(@CheckForNull List<?> thisList, Object other) {
        if (other == AbstractC1819jA.A04(thisList)) {
            return true;
        }
        ...
        boolean z2 = thisList instanceof RandomAccess;
        if (A00[7].charAt(4) == 'F') {
            throw new RuntimeException();
        }
        A00[0] = "rcHl15RocZyTXvjAlguJbKcz1P7sPU36";
        if (z2 && (list instanceof RandomAccess)) {
            for (int i2 = 0; i2 < size; i2++) {
                if (!BX.A01(thisList.get(i2), list.get(i2))) {
                    return false;
                }
            }
            return true;
        }
        return AbstractC2063nL.A0G(thisList.iterator(), list.iterator());
    }
}
