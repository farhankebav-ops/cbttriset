...
public abstract class BaseRequestOptions<T extends BaseRequestOptions<T>> implements Cloneable {
    ...
    private static boolean isSet(int i2, int i8) {
        return (i2 & i8) != 0;
    }
    ...
    public T apply(@NonNull BaseRequestOptions<?> baseRequestOptions) {
        if (this.isAutoCloneEnabled) {
            return (T) mo212clone().apply(baseRequestOptions);
        }
    ...
    public T decode(@NonNull Class<?> cls) {
        if (this.isAutoCloneEnabled) {
            return (T) mo212clone().decode(cls);
        }
    ...
    public T diskCacheStrategy(@NonNull DiskCacheStrategy diskCacheStrategy) {
        if (this.isAutoCloneEnabled) {
            return (T) mo212clone().diskCacheStrategy(diskCacheStrategy);
        }
    ...
    public T dontTransform() {
        if (this.isAutoCloneEnabled) {
            return (T) mo212clone().dontTransform();
        }
        this.transformations.clear();
        int i2 = this.fields;
        this.isTransformationRequired = false;
    ...
    @NonNull
    @CheckResult
    public T encodeQuality(@IntRange(from = FirebaseRemoteConfig.DEFAULT_VALUE_FOR_LONG, to = 100) int i2) {
    ...
    public T error(@Nullable Drawable drawable) {
        if (this.isAutoCloneEnabled) {
            return (T) mo212clone().error(drawable);
        }
    ...
    public T fallback(@Nullable Drawable drawable) {
        if (this.isAutoCloneEnabled) {
            return (T) mo212clone().fallback(drawable);
        }
    ...
    @NonNull
    @CheckResult
    public T frame(@IntRange(from = FirebaseRemoteConfig.DEFAULT_VALUE_FOR_LONG) long j) {
    ...
    public T onlyRetrieveFromCache(boolean z2) {
        if (this.isAutoCloneEnabled) {
            return (T) mo212clone().onlyRetrieveFromCache(z2);
        }
    ...
    public final T optionalTransform(@NonNull DownsampleStrategy downsampleStrategy, @NonNull Transformation<Bitmap> transformation) {
        if (this.isAutoCloneEnabled) {
            return (T) mo212clone().optionalTransform(downsampleStrategy, transformation);
        }
    ...
    @NonNull
    @CheckResult
    public T override(int i2, int i8) {
        if (this.isAutoCloneEnabled) {
            return (T) mo212clone().override(i2, i8);
        }
    ...
    public T placeholder(@Nullable Drawable drawable) {
        if (this.isAutoCloneEnabled) {
            return (T) mo212clone().placeholder(drawable);
        }
    ...
    public T priority(@NonNull Priority priority) {
        if (this.isAutoCloneEnabled) {
            return (T) mo212clone().priority(priority);
        }
    ...
    public T removeOption(@NonNull Option<?> option) {
        if (this.isAutoCloneEnabled) {
            return (T) mo212clone().removeOption(option);
        }
    ...
    public <Y> T set(@NonNull Option<Y> option, @NonNull Y y) {
        if (this.isAutoCloneEnabled) {
            return (T) mo212clone().set(option, y);
        }
    ...
    public T signature(@NonNull Key key) {
        if (this.isAutoCloneEnabled) {
            return (T) mo212clone().signature(key);
        }
    ...
    public T sizeMultiplier(@FloatRange(from = 0.0d, to = 1.0d) float f4) {
        if (this.isAutoCloneEnabled) {
            return (T) mo212clone().sizeMultiplier(f4);
        }
        if (f4 < 0.0f || f4 > 1.0f) {
    ...
    public T skipMemoryCache(boolean z2) {
        if (this.isAutoCloneEnabled) {
            return (T) mo212clone().skipMemoryCache(true);
        }
        this.isCacheable = !z2;
        this.fields |= 256;
        return (T) selfOrThrowIfLocked();
    ...
    public T theme(@Nullable Resources.Theme theme) {
        if (this.isAutoCloneEnabled) {
            return (T) mo212clone().theme(theme);
        }
        this.theme = theme;
        if (theme != null) {
            this.fields |= 32768;
    ...
    @NonNull
    @CheckResult
    public T timeout(@IntRange(from = FirebaseRemoteConfig.DEFAULT_VALUE_FOR_LONG) int i2) {
    ...
    public final T transform(@NonNull DownsampleStrategy downsampleStrategy, @NonNull Transformation<Bitmap> transformation) {
        if (this.isAutoCloneEnabled) {
            return (T) mo212clone().transform(downsampleStrategy, transformation);
        }
        downsample(downsampleStrategy);
    ...
    public T useAnimationPool(boolean z2) {
        if (this.isAutoCloneEnabled) {
            return (T) mo212clone().useAnimationPool(z2);
        }
    ...
    public T useUnlimitedSourceGeneratorsPool(boolean z2) {
        if (this.isAutoCloneEnabled) {
            return (T) mo212clone().useUnlimitedSourceGeneratorsPool(z2);
        }
    ...
    private T scaleOnlyTransform(@NonNull DownsampleStrategy downsampleStrategy, @NonNull Transformation<Bitmap> transformation, boolean z2) {
        T t3 = z2 ? (T) transform(downsampleStrategy, transformation) : (T) optionalTransform(downsampleStrategy, transformation);
        t3.isScaleOnlyOrNoTransform = true;
        return t3;
    }
    ...
    public T fallback(@DrawableRes int i2) {
        if (this.isAutoCloneEnabled) {
            return (T) mo212clone().fallback(i2);
        }
        this.fallbackId = i2;
    ...
    public T placeholder(@DrawableRes int i2) {
        if (this.isAutoCloneEnabled) {
            return (T) mo212clone().placeholder(i2);
        }
    ...
    @NonNull
    public T transform(@NonNull Transformation<Bitmap> transformation, boolean z2) {
        if (this.isAutoCloneEnabled) {
            return (T) mo212clone().transform(transformation, z2);
        }
    ...
    @NonNull
    public <Y> T transform(@NonNull Class<Y> cls, @NonNull Transformation<Y> transformation, boolean z2) {
        if (this.isAutoCloneEnabled) {
            return (T) mo212clone().transform(cls, transformation, z2);
        }
