...
public class DrawableTransformation implements Transformation<Drawable> {
    ...
    @NonNull
    public Resource<Drawable> transform(@NonNull Context context, @NonNull Resource<Drawable> resource, int i2, int i8) {
        BitmapPool bitmapPool = Glide.get(context).getBitmapPool();
        Drawable drawable = resource.get();
        Resource<Bitmap> resourceConvert = DrawableToBitmapConverter.convert(bitmapPool, drawable, i2, i8);
        if (resourceConvert != null) {
            Resource<Bitmap> resourceTransform = this.wrapped.transform(context, resourceConvert, i2, i8);
