...
final class DecodeHelper<Transcode> {
    ...
    public List<Key> getCacheKeys() {
        if (!this.isCacheKeysSet) {
            ...
            for (int i2 = 0; i2 < size; i2++) {
                ...
                for (int i8 = 0; i8 < loadData2.alternateKeys.size(); i8++) {
                    if (!this.cacheKeys.contains(loadData2.alternateKeys.get(i8))) {
                        this.cacheKeys.add(loadData2.alternateKeys.get(i8));
                    }
                }
            }
        }
    ...
    public <T> DataRewinder<T> getRewinder(T t3) {
        return this.glideContext.getRegistry().getRewinder(t3);
    }
    ...
    /* JADX WARN: Multi-variable type inference failed */
    public <R> void init(GlideContext glideContext, Object obj, Key key, int i2, int i8, DiskCacheStrategy diskCacheStrategy, Class<?> cls, Class<R> cls2, Priority priority, Options options, Map<Class<?>, Transformation<?>> map, boolean z2, boolean z7, DecodeJob.DiskCacheProvider diskCacheProvider) {
        ...
        this.isScaleOnlyOrNoTransform = z7;
    }
