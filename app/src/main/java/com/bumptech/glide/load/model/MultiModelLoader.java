...
class MultiModelLoader<Model, Data> implements ModelLoader<Model, Data> {
    ...
    public ModelLoader.LoadData<Data> buildLoadData(@NonNull Model model, int i2, int i8, @NonNull Options options) {
        ModelLoader.LoadData<Data> loadDataBuildLoadData;
        int size = this.modelLoaders.size();
        ArrayList arrayList = new ArrayList(size);
        Key key = null;
        for (int i9 = 0; i9 < size; i9++) {
            ModelLoader<Model, Data> modelLoader = this.modelLoaders.get(i9);
            if (modelLoader.handles(model) && (loadDataBuildLoadData = modelLoader.buildLoadData(model, i2, i8, options)) != null) {
