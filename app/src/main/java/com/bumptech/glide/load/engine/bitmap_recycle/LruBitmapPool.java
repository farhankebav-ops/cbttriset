...
public class LruBitmapPool implements BitmapPool {
    ...
    public static class ThrowingBitmapTracker implements BitmapTracker {
        ...
        public void add(Bitmap bitmap) {
            ...
            throw new IllegalStateException("Can't add already added bitmap: " + bitmap + " [" + bitmap.getWidth() + "x" + bitmap.getHeight() + C2300e4.i.e);
        }
    ...
    @NonNull
    private static Bitmap createBitmap(int i2, int i8, @Nullable Bitmap.Config config) {
        if (config == null) {
    ...
    @Nullable
    private synchronized Bitmap getDirtyOrNull(int i2, int i8, @Nullable Bitmap.Config config) {
        Bitmap bitmap;
        try {
            assertNotHardwareConfig(config);
            bitmap = this.strategy.get(i2, i8, config != null ? config : DEFAULT_CONFIG);
            if (bitmap == null) {
                if (Log.isLoggable(TAG, 3)) {
                    Log.d(TAG, "Missing bitmap=" + this.strategy.logBitmap(i2, i8, config));
                }
                this.misses++;
            } else {
                this.hits++;
            ...
            if (Log.isLoggable(TAG, 2)) {
                Log.v(TAG, "Get bitmap=" + this.strategy.logBitmap(i2, i8, config));
            }
            dump();
        } catch (Throwable th) {
            throw th;
    ...
    public Bitmap get(int i2, int i8, Bitmap.Config config) {
        Bitmap dirtyOrNull = getDirtyOrNull(i2, i8, config);
        if (dirtyOrNull == null) {
            return createBitmap(i2, i8, config);
        }
    ...
    public Bitmap getDirty(int i2, int i8, Bitmap.Config config) {
        Bitmap dirtyOrNull = getDirtyOrNull(i2, i8, config);
        return dirtyOrNull == null ? createBitmap(i2, i8, config) : dirtyOrNull;
    }
