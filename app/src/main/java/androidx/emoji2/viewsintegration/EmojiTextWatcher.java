...
final class EmojiTextWatcher implements TextWatcher {
    ...
    public void onTextChanged(CharSequence charSequence, int i2, int i8, int i9) {
        if (this.mEditText.isInEditMode() || shouldSkipForDisabledOrNotConfigured() || i8 > i9 || !(charSequence instanceof Spannable)) {
            return;
        }
        int loadState = EmojiCompat.get().getLoadState();
        if (loadState != 0) {
            if (loadState == 1) {
                EmojiCompat.get().process((Spannable) charSequence, i2, i2 + i9, this.mMaxEmojiCount, this.mEmojiReplaceStrategy);
                return;
            } else if (loadState != 3) {
                return;
            }
