...
public final class BrowserServiceFileProvider extends FileProvider {
    ...
    private static Uri generateUri(Context context, String str) {
        return new Uri.Builder().scheme("content").authority(context.getPackageName() + AUTHORITY_SUFFIX).path(a.m(FILE_SUB_DIR_NAME, str, FILE_EXTENSION)).build();
    }
    ...
    public static void grantReadPermission(@NonNull Intent intent, @Nullable List<Uri> list, @NonNull Context context) {
        ...
        for (int i2 = 1; i2 < list.size(); i2++) {
            clipDataNewUri.addItem(new ClipData.Item(list.get(i2)));
        }
        intent.setClipData(clipDataNewUri);
    }

    @NonNull
    public static q loadBitmap(@NonNull final ContentResolver contentResolver, @NonNull final Uri uri) {
    ...
    public static ResolvableFuture<Uri> saveBitmap(@NonNull Context context, @NonNull Bitmap bitmap, @NonNull String str, int i2) {
        ...
        sbX.append(Integer.toString(i2));
