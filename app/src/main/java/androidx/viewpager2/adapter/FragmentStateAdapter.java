...
public abstract class FragmentStateAdapter extends RecyclerView.Adapter<FragmentViewHolder> implements StatefulAdapter {
    ...
    private Long itemForViewHolder(int i2) {
        Long lValueOf = null;
        for (int i8 = 0; i8 < this.mItemIdToViewHolder.size(); i8++) {
            if (this.mItemIdToViewHolder.valueAt(i8).intValue() == i2) {
                if (lValueOf != null) {
                    throw new IllegalStateException("Design assumption violated: a ViewHolder can only be bound to one item at a time.");
                }
                lValueOf = Long.valueOf(this.mItemIdToViewHolder.keyAt(i8));
            }
        }
        return lValueOf;
    }
    ...
    public void gcFragments() {
        ...
        if (!this.mIsInGracePeriod) {
            ...
            for (int i8 = 0; i8 < this.mFragments.size(); i8++) {
                long jKeyAt2 = this.mFragments.keyAt(i8);
        ...
        for (String str : bundle.keySet()) {
            if (isValidKey(str, KEY_PREFIX_FRAGMENT)) {
                ...
                if (!isValidKey(str, KEY_PREFIX_STATE)) {
                    throw new IllegalArgumentException(a.l("Unexpected key in savedState: ", str));
                }
    ...
    public final Parcelable saveState() {
        ...
        for (int i8 = 0; i8 < this.mSavedStates.size(); i8++) {
            long jKeyAt2 = this.mSavedStates.keyAt(i8);
