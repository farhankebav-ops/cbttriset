...
public class MapEntryLite<K, V> {
    ...
    private MapEntryLite(WireFormat.FieldType fieldType, K k, WireFormat.FieldType fieldType2, V v2) {
        this.metadata = new Metadata<>(fieldType, k, fieldType2, v2);
        this.key = k;
        this.value = v2;
    }

    public static <K, V> int computeSerializedSize(Metadata<K, V> metadata, K k, V v2) {
        return FieldSet.computeElementSize(metadata.keyType, 1, k) + FieldSet.computeElementSize(metadata.valueType, 2, v2);
    }
    ...
    public static <K, V> void writeTo(CodedOutputStream codedOutputStream, Metadata<K, V> metadata, K k, V v2) throws IOException {
    ...
    private MapEntryLite(Metadata<K, V> metadata, K k, V v2) {
