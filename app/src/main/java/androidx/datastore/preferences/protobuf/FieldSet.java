...
final class FieldSet<T extends FieldDescriptorLite<T>> {
    ...
    public static final class Builder<T extends FieldDescriptorLite<T>> {
        ...
        public Object getRepeatedField(T t3, int i2) {
            if (this.hasNestedBuilders) {
                ensureIsMutable();
            }
            return replaceBuilder(getRepeatedFieldAllowBuilders(t3, i2), true);
        }

        public Object getRepeatedFieldAllowBuilders(T t3, int i2) {
            if (!t3.isRepeated()) {
        ...
        public void setRepeatedField(T t3, int i2, Object obj) {
            ensureIsMutable();
            if (!t3.isRepeated()) {
                throw new IllegalArgumentException("getRepeatedField() can only be called on repeated fields.");
            }
    ...
    public static <T extends FieldDescriptorLite<T>> SmallSortedMap<T, Object> cloneAllFieldsMap(SmallSortedMap<T, Object> smallSortedMap, boolean z2, boolean z7) {
        SmallSortedMap<T, Object> smallSortedMapNewFieldMap = SmallSortedMap.newFieldMap();
        int numArrayEntries = smallSortedMap.getNumArrayEntries();
        for (int i2 = 0; i2 < numArrayEntries; i2++) {
            cloneFieldEntry(smallSortedMapNewFieldMap, smallSortedMap.getArrayEntryAt(i2), z2, z7);
        }
        Iterator it = smallSortedMap.getOverflowEntries().iterator();
        while (it.hasNext()) {
            cloneFieldEntry(smallSortedMapNewFieldMap, (Map.Entry) it.next(), z2, z7);
        }
        return smallSortedMapNewFieldMap;
    }

    private static <T extends FieldDescriptorLite<T>> void cloneFieldEntry(Map<T, Object> map, Map.Entry<T, Object> entry, boolean z2, boolean z7) {
        T key = entry.getKey();
        Object value = entry.getValue();
    ...
    public static void writeField(FieldDescriptorLite<?> fieldDescriptorLite, Object obj, CodedOutputStream codedOutputStream) throws IOException {
        ...
        if (!fieldDescriptorLite.isPacked()) {
            while (i2 < size) {
                writeElement(codedOutputStream, liteType, number, list.get(i2));
                i2++;
            }
        } else {
            if (list.isEmpty()) {
            ...
            for (int i8 = 0; i8 < size; i8++) {
                iComputeElementSizeNoTag += computeElementSizeNoTag(liteType, list.get(i8));
            }
