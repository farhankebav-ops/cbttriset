...
public class ReactiveGuide extends View implements SharedValues.SharedValuesListener {
    ...
    private void changeValue(int i2, int i8, MotionLayout motionLayout, int i9) {
        ConstraintSet constraintSet = motionLayout.getConstraintSet(i9);
        constraintSet.setGuidelineEnd(i8, i2);
        ...
        if (id > 0 && (getParent() instanceof MotionLayout)) {
            ...
            if (!this.mAnimateChange) {
                if (!this.mApplyToAllConstraintSets) {
                    changeValue(i8, id, motionLayout, currentState);
                    return;
                }
                int[] constraintSetIds = motionLayout.getConstraintSetIds();
                while (i11 < constraintSetIds.length) {
                    changeValue(i8, id, motionLayout, constraintSetIds[i11]);
                    i11++;
                }
                return;
            }
    ...
    public ReactiveGuide(Context context, AttributeSet attributeSet, int i2, int i8) {
        super(context, attributeSet, i2);
        this.mAttributeId = -1;
        this.mAnimateChange = false;
        this.mApplyToConstraintSetId = 0;
