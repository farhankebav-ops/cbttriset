...
public class ConstraintLayoutStatistics {
    ...
    private String compare(DecimalFormat decimalFormat, ConstraintLayoutStatistics constraintLayoutStatistics, int i2) {
        String strR = a.r(a.x(fmt(decimalFormat, getValue(i2) * 1.0E-6f, 7), " -> "), fmt(decimalFormat, constraintLayoutStatistics.getValue(i2) * 1.0E-6f, 7), "ms");
        String strR2 = a.r(new StringBuilder(), WORD_PAD, geName(i2));
        return a.m("CL Perf: ", a.C(strR2.substring(strR2.length() - MAX_WORD), " = "), strR);
    }

    private String fmt(DecimalFormat decimalFormat, float f4, int i2) {
        StringBuilder sbR = androidx.camera.core.processing.util.a.r(new String(new char[i2]).replace((char) 0, ' '));
        sbR.append(decimalFormat.format(f4));
        String string = sbR.toString();
        return string.substring(string.length() - i2);
    }
    ...
    private String log(DecimalFormat decimalFormat, int i2) {
        ...
        String strR = a.r(new StringBuilder(), WORD_PAD, geName(i2));
        return a.m("CL Perf: ", a.C(strR.substring(strR.length() - MAX_WORD), " = "), strFmt);
    }
    ...
    private String compare(ConstraintLayoutStatistics constraintLayoutStatistics, int i2) {
        ...
        String strR = a.r(new StringBuilder(), WORD_PAD, geName(i2));
        return a.m("CL Perf: ", a.C(strR.substring(strR.length() - MAX_WORD), " = "), str);
    }

    private String log(int i2) {
        String string = Long.toString(getValue(i2));
        String strR = a.r(new StringBuilder(), WORD_PAD, geName(i2));
        return a.m("CL Perf: ", a.C(strR.substring(strR.length() - MAX_WORD), " = "), string);
    }
}
