...
class TouchResponse {
    ...
    public float dot(float f4, float f8) {
        return (f8 * this.mTouchDirectionY) + (f4 * this.mTouchDirectionX);
    }
    ...
    public void processTouchEvent(MotionEvent motionEvent, MotionLayout.MotionTracker motionTracker, int i2, MotionScene motionScene) {
        ...
        if (action == 1) {
            ...
            float[] fArr2 = this.mAnchorDpDt;
            float fAbs = f8 != 0.0f ? xVelocity / fArr2[c7] : yVelocity / fArr2[1];
            float f9 = !Float.isNaN(fAbs) ? (fAbs / 3.0f) + progress : progress;
            if (f9 == 0.0f || f9 == 1.0f || (i8 = this.mOnTouchUp) == 3) {
                if (0.0f >= f9 || 1.0f <= f9) {
        ...
        if (Math.abs((this.mTouchDirectionY * rawY) + (this.mTouchDirectionX * rawX)) > this.mDragThreshold || this.mDragStarted) {
            ...
            if (i10 != -1) {
                ...
                c9 = 0;
                f4 = progress2;
                this.mMotionLayout.getAnchorDpDt(i10, f4, this.mTouchAnchorX, this.mTouchAnchorY, this.mAnchorDpDt);
            } else {
                c8 = 1;
                f4 = progress2;
                c9 = 0;
            ...
            if (Math.abs(((this.mTouchDirectionY * fArr4[c8]) + (f11 * fArr4[c9])) * this.mDragScale) < 0.01d) {
                float[] fArr5 = this.mAnchorDpDt;
                fArr5[c9] = 0.01f;
                fArr5[c8] = 0.01f;
            }
            float fMax = Math.max(Math.min(f4 + (this.mTouchDirectionX != 0.0f ? rawX / this.mAnchorDpDt[c9] : rawY / this.mAnchorDpDt[c8]), 1.0f), 0.0f);
            ...
            if (fMax != progress3) {
                if (progress3 == 0.0f || progress3 == 1.0f) {
                    this.mMotionLayout.endTrigger(progress3 == 0.0f ? c8 : c9);
                }
                this.mMotionLayout.setProgress(fMax);
                motionTracker.computeCurrentVelocity(1000);
                this.mMotionLayout.mLastVelocity = this.mTouchDirectionX != 0.0f ? motionTracker.getXVelocity() / this.mAnchorDpDt[c9] : motionTracker.getYVelocity() / this.mAnchorDpDt[c8];
            } else {
    ...
    */
    ...
    public void scrollMove(float f4, float f8) {
        float progress = this.mMotionLayout.getProgress();
        if (!this.mDragStarted) {
            this.mDragStarted = true;
        ...
        float fMax = Math.max(Math.min(progress + (f10 != 0.0f ? (f4 * f10) / this.mAnchorDpDt[0] : (f8 * this.mTouchDirectionY) / this.mAnchorDpDt[1]), 1.0f), 0.0f);
        if (fMax != this.mMotionLayout.getProgress()) {
            this.mMotionLayout.setProgress(fMax);
        }
    }

    public void scrollUp(float f4, float f8) {
        int i2;
        this.mDragStarted = false;
        float progress = this.mMotionLayout.getProgress();
        this.mMotionLayout.getAnchorDpDt(this.mTouchAnchorId, progress, this.mTouchAnchorX, this.mTouchAnchorY, this.mAnchorDpDt);
        float f9 = this.mTouchDirectionX;
        float[] fArr = this.mAnchorDpDt;
        float f10 = f9 != 0.0f ? (f4 * f9) / fArr[0] : (f8 * this.mTouchDirectionY) / fArr[1];
        if (!Float.isNaN(f10)) {
            progress += f10 / 3.0f;
        }
        ...
        this.mMotionLayout.touchAnimateTo(i2, ((double) progress) >= 0.5d ? 1.0f : 0.0f, f10);
    }
    ...
    public void setDown(float f4, float f8) {
        this.mLastTouchX = f4;
        this.mLastTouchY = f8;
    }
    ...
    public void setTouchAnchorLocation(float f4, float f8) {
    ...
    public void setUpTouchEvent(float f4, float f8) {
        this.mLastTouchX = f4;
        this.mLastTouchY = f8;
        this.mDragStarted = false;
    }
