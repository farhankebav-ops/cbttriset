...
public class Helper {
    ...
    public Map<String, String> convertConfigToMap() {
        ...
        int i2 = 0;
        int i8 = 0;
        for (int i9 = 0; i9 < this.config.length(); i9++) {
            char cCharAt = this.config.charAt(i9);
            if (cCharAt == ':') {
                string = sb.toString();
                sb.setLength(0);
            } else if (cCharAt == ',' && i2 == 0 && i8 == 0) {
    ...
    public String toString() {
        StringBuilder sb = new StringBuilder(a.r(new StringBuilder(), this.name, ":{\n"));
        if (this.type != null) {
            sb.append("type:'");
