...
class FontRequestWorker {
                        ...
                        synchronized (FontRequestWorker.LOCK) {
                            try {
                                ...
                                for (int i8 = 0; i8 < arrayList3.size(); i8++) {
                                    arrayList3.get(i8).accept(typefaceResult);
                                }
                            } catch (Throwable th) {
                                throw th;
                            }
    ...
    public static Typeface requestFontSync(@NonNull final Context context, @NonNull final FontRequest fontRequest, @NonNull CallbackWithHandler callbackWithHandler, final int i2, int i8) {
        final String strCreateCacheId = createCacheId(fontRequest, i2);
        Typeface typeface = sTypefaceCache.get(strCreateCacheId);
        if (typeface != null) {
            callbackWithHandler.onTypefaceResult(new TypefaceResult(typeface));
