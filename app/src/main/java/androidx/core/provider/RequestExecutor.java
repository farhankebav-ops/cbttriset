...
class RequestExecutor {
    ...
    public static ThreadPoolExecutor createDefaultExecutor(@NonNull String str, int i2, @IntRange(from = FirebaseRemoteConfig.DEFAULT_VALUE_FOR_LONG) int i8) {
        ThreadPoolExecutor threadPoolExecutor = new ThreadPoolExecutor(0, 1, i8, TimeUnit.MILLISECONDS, new LinkedBlockingDeque(), new DefaultThreadFactory(str, i2));
    ...
    public static <T> T submit(@NonNull ExecutorService executorService, @NonNull Callable<T> callable, @IntRange(from = FirebaseRemoteConfig.DEFAULT_VALUE_FOR_LONG) int i2) throws InterruptedException {
        try {
