...
class RoundRectDrawableWithShadow extends Drawable {
    ...
    public RoundRectDrawableWithShadow(Resources resources, ColorStateList colorStateList, float f4, float f8, float f9) {
    ...
    private void buildComponents(Rect rect) {
        ...
        float f8 = SHADOW_MULTIPLIER * f4;
        this.mCardBounds.set(rect.left + f4, rect.top + f8, rect.right - f4, rect.bottom - f8);
        buildShadowCorners();
    }
    ...
    private void buildShadowCorners() {
        ...
        float f10 = f9 / (this.mShadowSize + f9);
        Paint paint = this.mCornerShadowPaint;
        float f11 = this.mCornerRadius + this.mShadowSize;
        int i2 = this.mShadowStartColor;
        Shader.TileMode tileMode = Shader.TileMode.CLAMP;
        paint.setShader(new RadialGradient(0.0f, 0.0f, f11, new int[]{i2, i2, this.mShadowEndColor}, new float[]{0.0f, f10, 1.0f}, tileMode));
        ...
        paint2.setShader(new LinearGradient(0.0f, f14, 0.0f, f15, new int[]{i8, i8, this.mShadowEndColor}, new float[]{0.0f, 0.5f, 1.0f}, tileMode));
        this.mEdgeShadowPaint.setAntiAlias(false);
    }

    public static float calculateHorizontalPadding(float f4, float f8, boolean z2) {
        if (!z2) {
            return f4;
        }
        return (float) (((1.0d - COS_45) * ((double) f8)) + ((double) f4));
    }

    public static float calculateVerticalPadding(float f4, float f8, boolean z2) {
        if (!z2) {
            return f4 * SHADOW_MULTIPLIER;
        }
        return (float) (((1.0d - COS_45) * ((double) f8)) + ((double) (f4 * SHADOW_MULTIPLIER)));
    }

    private void drawShadow(Canvas canvas) {
        Canvas canvas2;
        float f4 = this.mCornerRadius;
        float f8 = (-f4) - this.mShadowSize;
        float f9 = (this.mRawShadowSize / 2.0f) + f4 + this.mInsetShadow;
        float f10 = 2.0f * f9;
        boolean z2 = this.mCardBounds.width() - f10 > 0.0f;
        boolean z7 = this.mCardBounds.height() - f10 > 0.0f;
        ...
        if (z2) {
            canvas2 = canvas;
            canvas2.drawRect(0.0f, f8, this.mCardBounds.width() - f10, -this.mCornerRadius, this.mEdgeShadowPaint);
        } else {
            canvas2 = canvas;
        }
        ...
        canvas2.translate(rectF2.right - f9, rectF2.bottom - f9);
        canvas2.rotate(180.0f);
        canvas2.drawPath(this.mCornerShadowPath, this.mCornerShadowPaint);
        if (z2) {
            canvas2.drawRect(0.0f, f8, this.mCardBounds.width() - f10, (-this.mCornerRadius) + this.mShadowSize, this.mEdgeShadowPaint);
        }
        canvas2.restoreToCount(iSave2);
        int iSave3 = canvas2.save();
        RectF rectF3 = this.mCardBounds;
        canvas2.translate(rectF3.left + f9, rectF3.bottom - f9);
        canvas2.rotate(270.0f);
        canvas2.drawPath(this.mCornerShadowPath, this.mCornerShadowPaint);
        if (z7) {
            canvas2.drawRect(0.0f, f8, this.mCardBounds.height() - f10, -this.mCornerRadius, this.mEdgeShadowPaint);
        }
        canvas2.restoreToCount(iSave3);
        int iSave4 = canvas2.save();
        RectF rectF4 = this.mCardBounds;
        canvas2.translate(rectF4.right - f9, rectF4.top + f9);
    ...
    private void setShadowSize(float f4, float f8) {
        if (f4 < 0.0f) {
    ...
    public float getMinHeight() {
        float f4 = this.mRawMaxShadowSize;
        return (((this.mRawMaxShadowSize * SHADOW_MULTIPLIER) + this.mInsetShadow) * 2.0f) + (Math.max(f4, ((f4 * SHADOW_MULTIPLIER) / 2.0f) + this.mCornerRadius + this.mInsetShadow) * 2.0f);
    }
