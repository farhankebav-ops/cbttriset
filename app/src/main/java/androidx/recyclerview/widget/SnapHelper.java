...
public abstract class SnapHelper extends RecyclerView.OnFlingListener {
    ...
    private final RecyclerView.OnScrollListener mScrollListener = new RecyclerView.OnScrollListener() { // from class: androidx.recyclerview.widget.SnapHelper.1
        ...
        @Override // androidx.recyclerview.widget.RecyclerView.OnScrollListener
        public void onScrolled(RecyclerView recyclerView, int i2, int i8) {
            if (i2 == 0 && i8 == 0) {
                return;
            }
            this.mScrolled = true;
    ...
    private boolean snapFromFling(@NonNull RecyclerView.LayoutManager layoutManager, int i2, int i8) {
        RecyclerView.SmoothScroller smoothScrollerCreateScroller;
        int iFindTargetSnapPosition;
        if (!(layoutManager instanceof RecyclerView.SmoothScroller.ScrollVectorProvider) || (smoothScrollerCreateScroller = createScroller(layoutManager)) == null || (iFindTargetSnapPosition = findTargetSnapPosition(layoutManager, i2, i8)) == -1) {
            return false;
        }
    ...
    public int[] calculateScrollDistance(int i2, int i8) {
        this.mGravityScroller.fling(0, 0, i2, i8, Integer.MIN_VALUE, Integer.MAX_VALUE, Integer.MIN_VALUE, Integer.MAX_VALUE);
        ...
        if (layoutManager instanceof RecyclerView.SmoothScroller.ScrollVectorProvider) {
            return new LinearSmoothScroller(this.mRecyclerView.getContext()) { // from class: androidx.recyclerview.widget.SnapHelper.2
                ...
                public void onTargetFound(View view, RecyclerView.State state, RecyclerView.SmoothScroller.Action action) {
                    ...
                    int iCalculateTimeForDeceleration = calculateTimeForDeceleration(Math.max(Math.abs(i2), Math.abs(i8)));
                    if (iCalculateTimeForDeceleration > 0) {
                        action.update(i2, i8, iCalculateTimeForDeceleration, this.mDecelerateInterpolator);
                    }
                }
            };
        }
    ...
    public abstract int findTargetSnapPosition(RecyclerView.LayoutManager layoutManager, int i2, int i8);

    @Override // androidx.recyclerview.widget.RecyclerView.OnFlingListener
    public boolean onFling(int i2, int i8) {
        RecyclerView.LayoutManager layoutManager = this.mRecyclerView.getLayoutManager();
        if (layoutManager == null || this.mRecyclerView.getAdapter() == null) {
            return false;
        }
        int minFlingVelocity = this.mRecyclerView.getMinFlingVelocity();
        return (Math.abs(i8) > minFlingVelocity || Math.abs(i2) > minFlingVelocity) && snapFromFling(layoutManager, i2, i8);
    }
