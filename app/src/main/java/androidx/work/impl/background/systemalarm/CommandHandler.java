...
public class CommandHandler implements ExecutionListener {
    ...
    public static Intent createExecutionCompletedIntent(@NonNull Context context, @NonNull WorkGenerationalId workGenerationalId, boolean z2) {
        Intent intent = new Intent(context, (Class<?>) SystemAlarmService.class);
        ...
        synchronized (this.mLock) {
            try {
                ...
                if (this.mPendingDelayMet.containsKey(workGenerationalId)) {
                    ...
                    DelayMetCommandHandler delayMetCommandHandler = new DelayMetCommandHandler(this.mContext, i2, systemAlarmDispatcher, this.mStartStopTokens.tokenFor(workGenerationalId));
    ...
    private void handleExecutionCompleted(@NonNull Intent intent, int i2) {
        WorkGenerationalId workGenerationalId = readWorkGenerationalId(intent);
        boolean z2 = intent.getExtras().getBoolean(KEY_NEEDS_RESCHEDULE);
        Logger.get().debug(TAG, "Handling onExecutionCompleted " + intent + ", " + i2);
        lambda$runOnExecuted$1(workGenerationalId, z2);
    }

    private void handleReschedule(@NonNull Intent intent, int i2, @NonNull SystemAlarmDispatcher systemAlarmDispatcher) {
        Logger.get().debug(TAG, "Handling reschedule " + intent + ", " + i2);
        ...
        synchronized (this.mLock) {
            try {
                ...
                if (delayMetCommandHandlerRemove != null) {
                    delayMetCommandHandlerRemove.onExecuted(z2);
                }
            } catch (Throwable th) {
                throw th;
            }
